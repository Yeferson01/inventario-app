...
class InventorySyncRemoteDataSource {
  InventorySyncRemoteDataSource(this._client);

  final SupabaseClient _client;

  Future<CatalogUploadBatchResult> uploadAndProcessInventoryBatch({
    ...
    await _client.from('sync_batches').upsert(
      {
        ...
        'metadata': _decodeNullableJson(localBatch['metadata_json']) ??
            {
              'source': 'flutter_inventory_upload',
              'domain': 'inventory',
            },
        'created_at': now,
        'updated_at': now,
    ...
    final serverMutations = localMutations.map((mutation) {
      return {
        ...
        'metadata': _decodeNullableJson(mutation['metadata_json']) ??
            {
              'source': 'flutter_inventory_sync_mutation',
            },
        'created_at': now,
        'updated_at': now,
      };
