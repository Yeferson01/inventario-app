...
class ProductBarcodeSnapshotApplier
  ...
  Future<OperationalBootstrapPageApplyResult> applyPage({
    ...
    _support.validatePage(snapshot, page, 'product_barcodes');
    final seen = <String>[];
    for (final row in page.rows) {
      final remote = ProductOperationalBarcodeSnapshot.fromRow(row);
      if (remote.scope == 'business') {
        _support.validateBusiness(snapshot.businessId, remote.businessId!);
      }
      seen.add(remote.id);
      final local = await _localDao.getProductBarcode(remote.id);
      if (local == null) {
        if (row.state == OperationalBootstrapRecordState.present) {
          await _localDao.upsertProductBarcode(remote);
        }
        continue;
      }
      ...
      if (classification.canAcceptRemote) {
        await _localDao.upsertProductBarcode(
          remote,
          validateProductReference:
              row.state == OperationalBootstrapRecordState.present,
        );
        continue;
      }
  ...
  Future<void> finalizeDataset({
    ...
    for (final local
        in await _localDao.businessBarcodesForSweep(snapshot.businessId)) {
      ...
      if (classification.canAcceptRemote) {
        await _localDao.softInvalidateBusinessBarcode(
          id: id,
        ...
        await _support.openIssue(
          ...
          severity: classification.state ==
              ...
              : 'warning',
          message:
              'Local product barcode was preserved because it is dirty and absent from the authoritative snapshot.',
          classification: classification,
        );
      }
    }
  ...
  Future<void> _recordPreservedIssue({
    required String profileId,
    required OperationalBootstrapSnapshotPage snapshot,
    required ProductOperationalBarcodeSnapshot remote,
    required Map<String, dynamic> local,
    required CatalogEntityReconciliationClassification classification,
    required bool tombstone,
  }) async {
    final equivalent = _support.barcodeEquivalent(local, remote);
    if (!tombstone &&
        equivalent &&
    ...
    await _support.openIssue(
      ...
      message: tombstone
          ? 'Dirty local product barcode was preserved instead of applying a remote tombstone.'
          : 'Dirty local product barcode was preserved instead of applying the remote payload.',
      classification: classification,
    );
  }
}
