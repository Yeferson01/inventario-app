-- =========================================================
-- Fase 6.19C.3 - pull_product_catalog_delta
-- Objetivo:
-- - Descargar catálogo maestro liviano para SQLite/Drift local.
-- - No descargar imágenes pesadas.
-- - Incluir master_products, global_barcodes y business_barcodes.
-- - Soportar delta por updated_at y paginación por page_token.
-- - Enfoque offline-first: Flutter llama 1-2 veces al día, no por cada escaneo.
-- =========================================================
...
begin
  ...
  v_since := coalesce(p_since_updated_at, '1970-01-01 00:00:00+00'::timestamptz);
  v_server_time := clock_timestamp();

  v_page_token := coalesce(p_page_token, '{}'::jsonb);

  if jsonb_typeof(v_page_token) <> 'object' then
    raise exception 'page_token must be a JSON object';
  end if;

  v_token_updated_at := nullif(v_page_token->>'updated_at', '')::timestamptz;
  v_token_entity_type := nullif(v_page_token->>'entity_type', '');
  v_token_id := nullif(v_page_token->>'id', '');

  select coalesce(max(m.catalog_version), 0)
  into v_current_catalog_version
  from public.master_products_catalog m
  where p_include_deleted = true
  ...
  with records as (
    -- Master products: texto liviano, sin image_url completo.
    select
      ...
      jsonb_build_object(
        ...
        -- Importante: no enviamos image_url pesado.
        -- Solo metadata para lazy cache.
    ...
    where m.updated_at > v_since
      ...
      and (
        p_include_deleted = true
        or coalesce(m.sync_status, 'synced') = 'synced'
      )

    union all

    -- Global barcodes: GTIN/EAN/UPC compartidos.
    select
    ...
    where pb.scope = 'global'
      ...
      and (
        p_include_deleted = true
        or pb.deleted_at is null
      )

    union all

    -- Business barcodes: códigos locales del negocio, válidos para todas sus sucursales.
    select
  ...
  filtered as (
    ...
    from records r
    where
      v_token_updated_at is null
      or (
        r.updated_at > v_token_updated_at
        or (
          r.updated_at = v_token_updated_at
          and r.entity_type > coalesce(v_token_entity_type, '')
        )
        or (
          r.updated_at = v_token_updated_at
          and r.entity_type = coalesce(v_token_entity_type, '')
          and r.record_id::text > coalesce(v_token_id, '')
        )
      )
  ),
  ...
  select
    ...
    coalesce((select count(*)::integer from visible), 0) as returned_count,
...
'Pulls lightweight product catalog delta for offline-first Flutter local cache. Includes master products, global barcodes, and business-local barcodes. Does not include heavy images.';