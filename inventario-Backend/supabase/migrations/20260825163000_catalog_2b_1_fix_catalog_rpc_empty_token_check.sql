-- CATALOG-2B.1 - Fix the initial empty page-token check.
-- Forward-only replacement after PostgreSQL lint confirmed that
-- jsonb_object_length(jsonb) is unavailable. RPC and token contracts remain
-- unchanged.
