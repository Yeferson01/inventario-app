-- Fase 6.15F.5 - Patch sync conflict branch_id helper
-- Objetivo:
-- Corregir private.create_sync_conflict_for_mutation(...)
-- para insertar sync_conflicts.branch_id y cumplir el trigger:
-- sync_conflicts.branch_id must match sync_mutations.branch_id.
...
begin
  ...
  values (
    ...
    coalesce(v_mutation.client_sequence, 0),
    v_conflict_type,
    v_severity,
...
is 'Creates a sync conflict for a mutation, including required branch_id, operation, client_mutation_id, client_sequence and valid severity, then marks the mutation as conflict.';
