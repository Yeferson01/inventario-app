...
Deno.serve(async (request) => {
  ...
  try {
    businessId = requiredString(payload.business_id, "business_id");
    email = requiredString(payload.email, "email");
    roleId = requiredString(payload.role_id, "role_id");
  ...
  // The caller JWT invokes issuance. PostgreSQL, not the Edge Function,
  // decides members.invite and business/branch scope authority.
  const { data: issuedData, error: issuanceError } = await callerClient.rpc(
    "create_business_member_invitation",
    {
  ...
  // Service-role access starts only after the caller-authorized durable row
  // exists. The client never receives or supplies the service-role key.
  ...
  const { data: claimData, error: claimError } = await adminClient.rpc(
    "admin_claim_business_member_invitation_delivery",
  ...
  const redirectTo = optionalString(
    Deno.env.get("BUSINESS_MEMBER_INVITATION_REDIRECT_URL"),
  );
  ...
  const { error: deliveryError } = await adminClient.auth.admin.inviteUserByEmail(
    ...
    {
      data: {
        business_member_invitation_id: invitation.invitation_id,
  ...
  const { data: deliveryData, error: deliveryUpdateError } =
    await adminClient.rpc(
      "admin_update_business_member_invitation_delivery",
      {
        p_invitation_id: invitation.invitation_id,
