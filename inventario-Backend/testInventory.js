...
async function run() {
  try {
    ...
    // INVENTORY TEST
    // =========================================
    const { data: inventoryTest, error: inventoryTestError } = await supabase
      .from('inventory_movements')
      .select('*')

    console.log('\n===== INVENTORY TEST =====')
    if (inventoryTestError) {
      console.log('Error en Inventario:', inventoryTestError.message)
    } else {
      console.log('Movimientos de inventario:', inventoryTest)
    }
